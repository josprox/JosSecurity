...
if(isset($_POST['actualizar_info'])){
  ...
  $consulta = consulta_mysqli_where("password","table_users","id",$iduser);
  if(password_verify($password,(string) $consulta['password']) == TRUE){
    actualizar_datos_mysqli('table_users',"`name` = '$name', `email` = '$email', `phone` = '$phone', `fa` = '$fa', `type_fa` = '$type_fa'","id",$iduser);
  }
  header("refresh:1;");
}
...
if(isset($_POST['update_password'])){
  ...
  $row = consulta_mysqli_where("password","table_users","id",$iduser);
  ...
  if(password_verify($password, (string) $password_encrypt) == TRUE){
    if ($password_new == $password_repeat){
      ...
      actualizar_datos_mysqli('table_users',"`password` = '$password_encriptada'",'id',$iduser);
    }
  }
  $conexion-> close();
}
...
if(isset($_POST['2fadesactivar'])){
  ...
  $info = $cst -> where();
  if($info['fa'] == "A"){
    actualizar_datos_mysqli("table_users","`fa` = 'A', `type_fa` = 'correo', `two_fa` = ''","id",$iduser);
  }else{
    actualizar_datos_mysqli("table_users","`fa` = 'D', `type_fa` = 'correo', `two_fa` = ''","id",$iduser);
  }
  header("refresh:1;");
}
