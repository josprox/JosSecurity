...
<body>
    ...
    if(isset($_GET['check_user'])){
        ...
        $consulta_mysql->tabla = "table_check_users";
        $consulta_mysql->comparar = "url";
        ...
        if($checker["email"] == $_GET['correo']){
            ...
            $check = logins($_GET['correo'],$_GET['contra'],"table_users",$cookies);
            print_r($check);
            if($check == false){
                ?>
                <script>
        ...
        if(recaptcha() == TRUE){
            if(leer_tablas_mysql_custom("SELECT mht_table_users.name FROM mht_table_users WHERE mht_table_users.email = '$correo'") >=1 ){
                $consulta = consulta_mysqli_where("name, last_ip, phone","table_users","email","'$correo'");
        ...
        if($contra == $new_contra){
            $consulta_id = consulta_mysqli_where("id","table_users","email","'$correo'");
        ...
        if(recaptcha() == TRUE){
            ...
            if($contra == $contra_repeat){
                echo registro("table_users",$_POST['txtName'],$_POST['txtCorreo'],$_POST['txtPassword'],6, $factor);
                header("refresh:1;");
            }else{
                ?>
                <script>
